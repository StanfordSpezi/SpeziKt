...
class KeyValueStorageTest {
    ...
    @Before
    fun setup() {
        val storageFactory = KeyValueStorageFactory.create(context = InstrumentationRegistry.getInstrumentation().targetContext)
        encryptedStorage = storageFactory.create("test-encrypted-storage", KeyValueStorageType.ENCRYPTED)
        unencryptedStorage = storageFactory.create("test-unencrypted-storage", KeyValueStorageType.UNENCRYPTED)
    }
