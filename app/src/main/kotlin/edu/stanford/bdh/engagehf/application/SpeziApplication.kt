...
abstract class SpeziApplication : Application() {
    ...
    override fun onCreate() {
        ...
        var index = 0
        while (index < DependencyRegistry.moduleLoadOrder.size) {
            val moduleId = DependencyRegistry.moduleLoadOrder[index]
            ...
            if(module is edu.stanford.bdh.engagehf.application.modules.Module) {
                config.putAllInputChannels(module.getInputChannels())
                config.putAllOutputChannels(module.getOutputChannels())
            }
        ...
        moduleConfigs.addAll(config.getAdditionalModuleConfigs())
        println("Module configs: $moduleConfigs")
