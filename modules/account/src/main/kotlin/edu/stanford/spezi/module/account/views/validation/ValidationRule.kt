...
data class ValidationRule internal constructor(
    ...
    companion object {
        operator fun invoke(regex: Regex, message: String): ValidationRule =
            ValidationRule(rule = { regex.matchEntire(it) != null }, message = message)
        operator fun invoke(copy: ValidationRule, message: String): ValidationRule =
            ValidationRule(rule = copy.rule, message = message)
    }

    val intercepting: ValidationRule
        get() =
        ValidationRule(id, rule, message, CascadingValidationEffect.INTERCEPT)
