...
- **Top-level resolution helpers** — `dependency<T>()` / `optionalDependency<T>()` return `Lazy<T>` delegates, and `requireDependency<T>()` / `requireOptionalDependency<T>()` resolve eagerly against the global graph. Used throughout modules to inject collaborators.
